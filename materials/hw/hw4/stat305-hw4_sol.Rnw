...
out<-lm(failure_time~weight, data = dw)
@

\begin{center}
<<fig.width=5, height=4, out.width='.5\\linewidth', echo=FALSE, message=FALSE, warning= FALSE, fig=TRUE>>=
  library(ggplot2)
...
\begin{enumerate}
      ...
      \begin{align*}
         b_1 &= \frac{ \sum_{i = 1}^n x_i y_i - n \bar{x} \bar{y} }{ \sum_{i = 1}^n x_i^2 - n \bar{x}^2 } \\
             ...
             &= \Sexpr{ ( (1699) - (50) * (64/50) * (1795/50) )/( 107 -  50 * (64/50)^2 ) } \\
      \end{align*} 
      and with $b_1$ we can find the value fo $b_0$: 
      \begin{align*}
         b_0 &= \bar{y} - b_1 \bar{x} \\
             &= (1996/50) - (\Sexpr{ ( (1699) - (50) * (64/50) * (1795/50) )/( 107 - 50 * (64/50)^2 ) })  (64/50) \\
             &= \Sexpr{(1699/50) - ( ( (1699) -  (50) * (64/50) * (1795/50) )/( 107 - 50 * (64/50)^2 ) )* (64/50) } \\
      \end{align*} 
      ...
      \item Find and \underline{interpret} the value of $R^2$ for the fitted linear relationship.[5 pts]
       ...
       Since we are using a linear relationship, we can get $R^2$ from $r$ (slides 29 and 51 of chapter 4 slides):
            ...
            r &= \frac{\sum_{i=1}^n x_i y_i - n \bar{x} \bar{y}}{\sqrt{\left(\sum_{i=1}^n x_i^2 - n \bar{x}^2\right)\left(\sum_{i=1}^n y_i^2 - n\bar{y}^2\right)}} \\
              ...
              &= \Sexpr{ ( (1699) - (50) * (64/50) * (1795/50) )/(sqrt( (107 - 50 * (64/50)^2 ) *(79777 - (50) * (1795/50)^2 ))) } \\
            \end{align*} 
            So $R^2 = (r)^2 = \Sexpr{( ( (1699) - (50) * (64/50) * (1795/50) )/(sqrt( (107 - 50 * (64/50)^2 ) *(79777 - (50) * (1795/50)^2 ))) )^2}$\\
      ...
      \item Using the fitted line, provide a predicted value of travel distance to failure when the weight exceeding the guidelines is 3.4 tonnes.[5 pts]
               ...
               $$ \hat{y} = 64.53 -23.86(3.4) = \Sexpr{64.53 -  23.86*(3.4)} $$
      ...
      Using the formula for residuals on page 36-37 of chapter 4 slides, 
            $$ e_{i} = y_i - \hat{y_i}= 0.37 - (-16.594)= \Sexpr{0.37 - (-16.594)} $$
...
\vspace{0.3cm}
       ...
       \item Find and \underline{interpret} the value of $R^2$ for the fitted quadratic relationship.[5 pts]
             ...
             $$R^2 = 1 - SSE/SSTO = 1 - (1311.073/14540.720) = \Sexpr{1 - (1311.073/14540.720)}$$
   ...
   $$ \hat{y} = 16.27602 + 4.6604349 (3.4) - 10.2775 (3.4)^2 = \Sexpr{ 16.27602 + 4.6604349 * (3.4) - 10.2775 * (3.4)^2 }$$
...
<<echo=FALSE>>=
    polyols <- read_csv("C:/Users/4M1N/Desktop/stat305_s2020/materials/data/polyols.csv")
    
#kable(polyols, col.names = c("Pot temperature (°C)", "Average molecular weight"), format = "latex")
@
\begin{center}
\begin{tabular}{r|r}
    \hline
...
%-- : R code (Code in Document)
<<fig=TRUE, echo=FALSE, include=TRUE, height=4>>=
out<-lm(weight~temp, data = polyols)
df<-data.frame("Residuals"= out$residuals, "Pot.Temperature"= polyols$temp) 
p<-df%>% ggplot(aes(x = Pot.Temperature, Residuals)) + geom_point() + geom_hline(yintercept = 0)
p
@
...
p1<-df %>% ggplot(aes(x = Residuals)) + geom_histogram( binwidth = 30, color="white")
p1
...
<<fig=TRUE, echo=FALSE, height=4>>=
p2<-df %>% ggplot(aes(sample=Residuals)) + stat_qq() + stat_qq_line() + ylab("Data quantiles") + xlab("Theoretical Normal Quantiles")+ coord_flip() 
p2
@
...
outquad<-lm(weight~ temp + temp2, data=data)
...
<<fig=TRUE, echo=FALSE, height=4>>=
df<-data.frame(outquad$residuals, data$temp)
names(df)<-c("residuals", "temperature")
q1<-df %>% ggplot(aes(temperature, residuals)) + geom_point() + geom_hline(yintercept = 0)
q2<-df %>% ggplot(aes(sample=residuals)) + stat_qq()+ stat_qq_line() + ylab("Data quantiles") + xlab("Theoretical Normal Quantiles")+ coord_flip() 
q1
