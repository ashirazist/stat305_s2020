...
library(tidyverse)
compound_1<- c(3.03 , 5.53 , 5.60 , 9.30 , 9.92 , 12.51 , 12.95 , 15.21 , 16.04 , 16.84)
compound_2<- c(3.19 ,4.26 , 4.47,4.53,4.67,4.69,5.78,6.79,9.37,12.75)

data.frame(compound_1, compound_2) %>% gather(key = "compound" ,value = "Time to failure") %>% mutate(compound= as.factor(compound)) %>%  ggplot(aes(x= compound, y = `Time to failure`)) + geom_boxplot(fill = "white", colour = "#3366FF") +coord_flip() + theme(legend.position="bottom") 
...
<<echo=FALSE>>=
df<-data.frame(compound_1, compound_2)
...
<<fig=TRUE, echo=FALSE, height=4>>=
df<-df %>% 
  dplyr::select(compound_1) %>%
  arrange(compound_1) %>% 
  mutate(i = 1:n(),
         `$\\frac{i-.5}{10}$` = (i -.5)/n()) %>%
  dplyr::select(2, 3, 1) %>%
  mutate(`Standard Normal quantiles` = qnorm((i-.5)/n()))
   
   
df %>% ggplot(aes(x = `compound_1`, y = `Standard Normal quantiles`))+ geom_point(color="red")+ xlab("Compound 1")+ ylab("Standard Normal") +geom_smooth(method = "lm")
@
...
df<-data.frame("Suplier I"= c(0.52,0.37,0.92,2.89,3.62,0.65), "Suplier II"= c(0.89,0.99,1.45,1.47,1.58,2.27))
df  %>% ggplot(aes(Suplier.I, Suplier.II))+ geom_point()
@
...
df.new %>% ggplot(aes(x = Suplier.I, y = `Standard Normal quantiles`))+ geom_point(color="red")+ xlab("Compound 1")+ ylab("Standard Normal") +geom_smooth(method = "lm")
@
...
df.new2 %>% ggplot(aes(x = Suplier.II, y = `Standard Normal quantiles`))+ geom_point(color="red")+ xlab("Compound 1")+ ylab("Standard Normal") +geom_smooth(method = "lm")
@
\end{center}
   The data from suplier II seems to have a bell-shaped distribution because it  follow a normal distribution as the points do  lie around the straight line in normal QQ-plot.
   \end{enumerate}
