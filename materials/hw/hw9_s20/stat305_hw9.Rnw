...
A process engineer is faced with the question, "How should gears be loaded into a continuous carburizing furnace in order to minimize distortion during heat treating?" The engineer conducts a well-thought-out study and obtains the runout values for \Sexpr{gears %>% filter(Arrangement == "laid") %>% nrow()} gears laid and \Sexpr{gears %>% filter(Arrangement == "hung") %>% nrow()} gears hung.
...
<<echo=FALSE>>=
    ...
    # knitr::kable(t(bushings)[, 1:8], format = "latex")
    # knitr::kable(t(bushings)[, 9:16], format= "latex")
@
...
<<echo=FALSE, eval=FALSE>>=
    polyols <- read.csv("./materials/data/polyols.csv")
    
    knitr::kable(polyols, col.names = c("Pot temperature (°C)", "Average molecular weight"), format = "latex")
@
\begin{centering}
\begin{tabular}{r|r}
