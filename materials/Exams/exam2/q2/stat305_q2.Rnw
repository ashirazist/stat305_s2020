...
<<echo=FALSE, cache=FALSE, include = TRUE>>=
   ...
   link<-"C:/Users/4M1N/Box Sync/Courses/Fall 2019/STAT 305/materials/exams/exam2-f19/q2/DK.csv"
   d <- read.csv(link)
...
\begin{center}
<<fig.width=5, fig.height=4, out.width='.5\\linewidth', echo=FALSE, fig=TRUE>>=
   
   df<-data.frame(d)
  p<- df %>% ggplot(aes(actual, predicted))+ geom_point()+ xlab("Person's Actual Score on Test")+ ylab("Predicted")+
