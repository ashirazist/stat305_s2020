...
\begin{parts}
   ...
   \part[10] Conduct a hypothesis test at the $\alpha = 0.05$ significance level for the claim that the true difference between the true means of the two O-rings ($\mu_X- \mu_Y$)  are significantly different.\\
\emph{Note: Write down all six steps for full credit.}
\end{parts}
