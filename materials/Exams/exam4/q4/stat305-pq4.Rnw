...
\begin{questions}
\question
In an effort to understand smartphone use, the American Association of Psychologists gained access to a single day's data on 625 smartphone users. They found that the smartphone users sampled spent an average of \Sexpr{ round(4.5*60, 2)} minutes using their phone on that day with a standard deviation of phone use was \Sexpr{ 32.15} minutes.
...
\begin{center}
\Sexpr{paste(dwnld_speeds,collapse=", ")}
\end{center}
